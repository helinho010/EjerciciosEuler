let cadenaDe1000Digitos:string = "";

function separacionDigitosAdyacentes(cantidadDigitos:number, cadena:string) : Array<string> 
{
    let subcadenaDe13Digitos: Array<string> = [];
    let posInicial: number = 0;
    let parteCadena:string = "";

    while (posInicial < cadena.length ) 
    {
        parteCadena = cadenaDe1000Digitos.substr(posInicial,cantidadDigitos); 
        if (parteCadena.indexOf("0") == -1) 
        {
            subcadenaDe13Digitos.push(parteCadena);
        }
        posInicial += 1;   
    }

    return subcadenaDe13Digitos;
}

function calcularProductoMayor(subcadenaDe13Digitos:Array<string>):Array<string> 
{
    let productoMayor: number = 1;
    let cadenaProductoMayor:string = "";
    let cantidadDigitos:number = 0;
    let resultado:Array<string> = [];

    for (let index = 0; index < subcadenaDe13Digitos.length; index++) 
    {
        let digito:number = 0;
        let nuevacadena:string = subcadenaDe13Digitos[index];
        let productoDigitos = 1;

        for (let i = 0; i < subcadenaDe13Digitos[index].length; i++) 
        {
            digito = parseInt(nuevacadena.substr(i,1),10);
            productoDigitos *= digito;
        }
        
        if (productoMayor <= productoDigitos) 
        {
            productoMayor = productoDigitos;
            cadenaProductoMayor = nuevacadena;
        }
    }
    resultado.push("Digitos Adyacentes : " + cadenaProductoMayor);
    resultado.push("Producto de todos los digitos :" + productoMayor.toString());
    return resultado;
}

console.time();
console.log(calcularProductoMayor(separacionDigitosAdyacentes(4,cadenaDe1000Digitos)));
console.timeEnd();

console.time();
console.log(calcularProductoMayor(separacionDigitosAdyacentes(13,cadenaDe1000Digitos)));
console.timeEnd();